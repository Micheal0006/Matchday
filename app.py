...
@app.route("/api/teams")
def api_teams():
    code = request.args.get("competition", "")
    if not code:
        return jsonify({"error": "competition code required"}), 400
    try:
        return jsonify({"teams": get_teams(code)})
...
def api_predict():
    ...
    home_name = request.args.get("home_name", "Home")
    away_name = request.args.get("away_name", "Away")
    ...
    try:
        home_matches = get_team_matches(home_id, limit=20)
        away_matches = get_team_matches(away_id, limit=20)

        home_form = compute_form(home_id, home_matches)
        away_form = compute_form(away_id, away_matches)
        h2h = compute_h2h(home_id, away_id, home_matches)

        result = run_prediction(home_form, away_form, h2h)
        result["home"] = {"name": home_name, **home_form}
        result["away"] = {"name": away_name, **away_form}
